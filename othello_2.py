...
x_corner_multiplier = {"x":-1, "o":1, ".": 0}
o_corner_multipler = {"o": -1, "x": 1, "." : 0}
...
def score(board, x_moves, o_moves):
    ...
    if len(x_moves) == 0:
        score -= 50
    for c in corners:
        x_corner = (corner := board[c]) == "x"
        o_corner = corner == "o"
        for neighbor in corners[c]:
            if x_corner:
                score -= 10 * char_to_val[board[neighbor]] * x_corner_multiplier[board[neighbor]]
            elif o_corner:
                score -= 10 * char_to_val[board[neighbor]] * o_corner_multipler[board[neighbor]]
            else:
...
def max_step(board, depth, alpha, beta):
    ...
    if depth == 0 or (len(possible) == 0 and len(possible_o) == 0):
        ...
        for index in possible:
            move = make_move(board, "x", index) #ALPHA/BETA PRUNING HERE
            step = min_step(move, depth-1, alpha, beta)
            results.append(step)
            alpha = max(step, alpha)
            if alpha >= beta:
                break
        return max(results)
...
def min_step(board, depth, alpha, beta):
 ...
 #       return min([max_step(move, depth-1) for move in moves])
        results = []
        for index in possible:
            move = make_move(board, "o", index) #ALPHA/BETAPRUNING HERE
            step = max_step(move, depth-1, alpha, beta)
            results.append(step)
            beta = min(step, beta)
            if alpha >= beta:
                break
...
def find_next_move(board, player, depth):
    ...
    if player == "x":
        ...
        for move in move_list:
            move = make_move(board, "x", move) #ALPHA/BETA PRUNING HERE
            step = min_step(move, depth, alpha, beta)
            scores.append(step)
            alpha = max(step, alpha)
        ...
        scores = []
        for move in move_list:
            move = make_move(board, "o", move) #ALPHA/BETA PRUNING HERE
            step = max_step(move, depth, alpha, beta)
            scores.append(step)
            beta = min(step, beta)
